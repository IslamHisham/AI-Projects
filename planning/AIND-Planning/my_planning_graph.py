...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level-1]:
            for child_s_node in action_node.effnodes:
                self.s_levels[level].add(child_s_node)
                child_s_node.parents.add(action_node)
                action_node.children.add(child_s_node)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for s1_parent in node_s1.parents:
            for s2_parent in node_s2.parents:
                if not  s1_parent.is_mutex(s2_parent):#if one doesn't match return false
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for level,s_node in enumerate(self.s_levels):
                if goal_node in self.s_levels[level]:
                    level_sum+=level
                    break #get out of this loop to start searching for the second goal node
        return level_sum
